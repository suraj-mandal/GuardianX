...
import {performLoginByPhone} from "@/lib/api.ts";
import {UserDto} from "@/models/user-dto.ts";
import LoginByPhoneSchema from "@/login/schema/login-by-phone-schema.ts";
...
export default function LoginByPhone() {
    ...
    const handleFormSubmit = (data: IUserLoginInputs) => {
        setLoading(true);
        toast.promise(performLoginByPhone(data.phoneNumber, data.password), {
            loading: "Logging in",
    ...
    return (
        <div className="flex justify-center items-center font-inter mt-8">
